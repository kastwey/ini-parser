...
namespace IniFileParser.Tests.Unit
{
	...
	public class IniParserFileTest
	{
		...
		{
			Assert.That(() => iniParser.ReadFile(strBadINIFilePath), Throws.InstanceOf<ParsingException>());
		}
		...
		{
			Assert.That(() => iniParser.ReadFile(strBadSectionINIFilePath), Throws.InstanceOf<ParsingException>());
		}

		[Test, Description("Checks bad formed INI file: Two keys in the same section with same name")]
		public void CheckCollideKeysNames()
		{
			Assert.That(() => iniParser.ReadFile(strBadKeysINIFilePath), Throws.InstanceOf<ParsingException>());
		}
	}
}
